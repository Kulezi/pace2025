...
struct BenchmarkInfo {
    ...
    std::vector<std::chrono::duration<double, std::milli>> rule_time;
    std::vector<std::chrono::duration<double, std::milli>> rule_branch_time;
    std::chrono::duration<double, std::milli> treewidth_decompositon_time;
    std::chrono::duration<double, std::milli> treewidth_calculation_time;

    BenchmarkInfo() = default;
    BenchmarkInfo(std::vector<RRules::Rule> rules, std::vector<RRules::Rule> rules_branch)
        : rule_time(rules.size()), rule_branch_time(rules_branch.size()) {}
};
#endif
...
struct Exact {
 ...
 TreeDecomposition td(g);
        benchmark_info.treewidth_decompositon_time += std::chrono::high_resolution_clock::now() - start;
        ...
        benchmark_info.max_encountered_treewidth = std::max(benchmark_info.max_encountered_treewidth,
                                                            td.width());
#else
        TreeDecomposition td(g);
        c = std::vector<std::vector<int>>(td.n_nodes(), std::vector<int>());
