...
void check_multilevel_partition_invariants(const Tail& tail, const Head& head,
    ...
    while (!open_cells.empty()) {
        ...
        if (current_cell.bag_size() > max_closed_bag_size) {
            ...
            for (int x = 0; x < interior_node_count; ++x)
                if (!is_in_sub_separator(x)) nodes_of_representative[uf(x)].push_back(x);
...
DSHunter::TreeDecomposition make_tree_decompostion_of_multilevel_partition(
    ...
    const std::vector<Cell>& cell_list, const std::vector<int>& reverse_mapping) {
...
DSHunter::TreeDecomposition tree_decompostion_of_order(ArrayIDIDFunc tail, ArrayIDIDFunc head,
    ...
    for (auto& b : nodes_in_bag)
        if ((int)b.size() > maximum_bag_size) maximum_bag_size = b.size();
...

    {
        ...
        for (int b = 0; b < bag_count; ++b) {
            ...
            for (auto x : nodes_in_bag[b]) {
                vector<int> tmp;
                std::set_union(bags_of_node[x].begin(), bags_of_node[x].end(),
                               neighbor_bags.begin(), neighbor_bags.end(), std::back_inserter(tmp));
        ...
        for (int b = 0; b < bag_count; ++b) {
            if (!in_tree(b)) {
                if (b != 0) output_backbone_edge(0, b);
                in_tree.set(b, true);
...
void test_new_order(const ArrayIDIDFunc& order, DSHunter::TreeDecomposition& bestDecomposition,
                    const vector<int>& reverse_mapping) {
...

        {
            try {
                ...
                if (node_count > 500000) {
                    ...
                    compute_multilevel_partition(tail, head, flow_cutter::ComputeSeparator(config),
                                                 best_decomposition.width,
                                                 on_new_multilevel_partition);
                }
...

                {
                    ...
                    for (int i = 2;
                         ...
                         best_decomposition.width > treewidth_threshold;
                         ++i) {
                        config.random_seed = rand_gen();
