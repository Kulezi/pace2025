...
void BranchingSolver::branch(const Instance &g, int v, std::vector<int> &best_ds) {
    ...
    // good.
    if (g.deg(v) != 1) solve(take(g, v), best_ds);

    std::vector<int> to_take;
    for (auto [u, s] : g[v].adj)
        if (s == EdgeStatus::FORCED) to_take.push_back(u);
...
Instance BranchingSolver::take(Instance g, int v) {
    g.take(v);
    DS_TRACE(std::cerr << std::string(level, ' ') << "took" << dbg(v) << dbgv(g.ds) << std::endl);
    return g;
}

Instance BranchingSolver::take(Instance g, std::vector<int> to_take) {
