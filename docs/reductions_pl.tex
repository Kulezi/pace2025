...
\subsubsection{Redukcja różowego sąsiedztwa}
...
\subsubsection{Redukcja zdominowanego sąsiedztwa}
\emph{Jeśli istnieje nieróżowy wierzchołek $v$ to jeśli:
\begin{itemize}
 \item całe $N(v)$ jest zdominowane
 \item $N(v)$ zawiera jakiś nieróżowy wierzchołek
 \item zachodzi jedno z: \begin{itemize}
    \item z $v$ nie wychodzą żadne czerwone krawędzie
    \item z $v$ wychodzi dokładnie jedna czerowna krawędź do nieróżowego wierzchołka
 \end{itemize}
\end{itemize}}
to pokoloruj $v$ na różowo.
...
Czyli pod założeniem wyczerpania poprzednich redukcji bez zmian, wpp. wystarczy nie aplikować redukcji jeśli zachodzą wyżej wymienione sytuacje.
